...
{{- $netManagerImage:="cnrancher/network-manager:v0.7.22-cn6" }}
{{- $metadataImage:="cnrancher/metadata:v0.10.4-cn1" }}
...
services:
  network-manager:
    ...
    network_mode: host
    pid: host
    command: plugin-manager --disable-cni-setup --metadata-address 169.254.169.250 --arpsync-interval ${ARP_SYNC_INTERVAL} --disable-vxlanhostns-sync
    environment:
